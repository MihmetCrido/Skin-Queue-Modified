...
local common = GetModConfigData("common")
local classy = GetModConfigData("classy")
local spiffy = GetModConfigData("spiffy")
    ...
    elseif #queue > 0 then
        for i = 1, #queue do
            if i == Queuer.count then
                print(i, queue[i].key, queue[i].val, queue[i].buy and "buying..." or "selling...")
            elseif i < Queuer.count then
                print(i, queue[i].key, queue[i].val, queue[i].buy and "bought" or "sold")
            else
...
HUD.create = function(parent)
    ...
    local SetString_Old = self.SetString or (function()
    end)
...
HUD.onQueueUpdate = function(timetaken)
    ...
    if current then
        lines[#lines + 1] = (current.buy and "Buying " or "Selling ") .. current.key .. " for " .. current.val ..
                                " spools..."
    end
    ...
    lines[#lines + 1] = Queuer.count .. "/" .. #Queuer.queue .. " (" ..
                            math.floor(100 * (Queuer.count / #Queuer.queue) + 0.5) .. "%) items"
    ...
    if HUD.timetaken > 0 then
        lines[#lines + 1] = HUD.getETR(HUD.timetaken * (#Queuer.queue - (Queuer.count - 1))) .. " etr"
    end
...
HUD.getETR = function(x)
    ...
    elseif secs > 86400 then
        return math.floor(secs / 3600) .. ":" .. G.os.date("!%M:%S", secs)
    end
...
DupesPopup.create = function(duplicates)
    ...
    if not G.TheInput:ControllerAttached() then
        self.back_button = self:AddChild(TEMPLATES.BackButton(function()
            self:Close()
        end))
    end

    self.title = self:AddChild(Text(G.HEADERFONT, 28, #self.dupes .. " Duplicates", G.UICOLOURS.GOLD_SELECTED))
    ...
    self.button = self:AddChild(TEMPLATES.StandardButton(function()
        self:Accept()
    end, "Unravel All", {200, 60}))
    self.button:SetPosition(290, -240)

    self.total = self:AddChild(Text(G.HEADERFONT, 28, nil, G.UICOLOURS.GOLD_SELECTED))
    self.total:SetString(G.TheInventory:GetCurrencyAmount() .. " + " .. DupeSeller.getDupesTotalWorth(self.dupes) ..
                             " = " .. G.TheInventory:GetCurrencyAmount() + DupeSeller.getDupesTotalWorth(self.dupes) ..
                             " Spools")
...
DupesPopup.buildGrid = function(self)
    ...
    local function scrollWidgetCtorFn(context, index)
        ...
        w.backing = w:AddChild(TEMPLATES.ListItemBackground(235, 120, function()
        end))
        ...
        w.name = w.backing:AddChild(Text(G.HEADERFONT, 20, nil, G.UICOLOURS.GOLD_SELECTED))
        ...
        w.value = w.backing:AddChild(Text(G.CHATFONT_OUTLINE, 20, nil, G.UICOLOURS.WHITE))
    ...
    local function scrollWidgetApplyFn(context, widget, data, index)
        ...
        widget.item:SetItem(G.GetTypeForItem(data), data)

        widget.name:SetMultilineTruncatedString(G.GetSkinName(data), 2, 110)
        widget.name:SetPosition(48, 18)

        widget.value:SetString("×" .. G.TheItems:GetBarterSellPrice(data))
...
local BuildDialog_Old = BarterScreen._BuildDialog or (function()
end)
...
BarterScreen._BuildDialog = function(self, ...)
    ...
    if Queuer.getItemCount(self.item_key) == 1 then
        ...
        dialog.body:SetString("WARNING: THIS IS YOUR LAST ONE\n\n" .. dialog.body:GetString():match("^(.*)\n"))

    elseif Queuer.getItemCount(self.item_key) <= 0 then

        dialog.body:SetString("You have no " .. G.GetSkinName(self.item_key) ..
...
local BuildItemsSummary_Old = PlayerSummaryScreen._BuildItemsSummary or (function()
end)
...
PlayerSummaryScreen._BuildItemsSummary = function(self, ...)
    ...
    local UpdateItems_Old = root.UpdateItems or (function()
    end)
    ...
    root.selldupes = root:AddChild(TEMPLATES.StandardButton(function()
        G.TheFrontEnd:PushScreen(DupesPopup.create(DupeSeller.getAllDupes()))
    end, "Unravel All Duplicates", {200, 40}))
...
Queuer.getItemCount = function(key, n)
    return (n or G.GetOwnedItemCounts()[key] or 0) - (Queuer.itemQueueCount[key] or 0)
end
...
Queuer.unravelFn = function(key, val)
    ...
    if not item_id then
        ...
        local server_error = PopupDialog(G.STRINGS.UI.TRADESCREEN.SERVER_ERROR_TITLE,
    ...
    G.TheItems:BarterLoseItem(item_id, val, function(success, status)
...
Queuer.unravelComplete = function(success, status, sounds, key)
    ...
    if not success then
        ...
        local server_error = PopupDialog(G.STRINGS.UI.BARTERSCREEN.FAILED_TITLE, G.STRINGS.UI.BARTERSCREEN.FAILED_BODY,
            {{
    ...
    elseif screen.doodad_count then
        screen.doodad_count:SetCount(G.TheInventory:GetCurrencyAmount(), true)
    end
...
Queuer.weaveFn = function(key, val)
    ...
    G.TheItems:BarterGainItem(key, val, function(success, status)

        G.scheduler:ExecuteInTime(0, function()
            Queuer.weaveComplete(success, status, {"dontstarve/HUD/Together_HUD/collectionscreen/weave",
                                                   "dontstarve/HUD/Together_HUD/collectionscreen/unlock"}, key)
...
Queuer.getScreen = function()
    ...
    if not (G.TheFrontEnd and G.TheFrontEnd.screenstack and #G.TheFrontEnd.screenstack > 0) then
        return {}
    end
...
DupeSeller.getAllDupes = function()
    ...
    for k, v in pairs(G.GetOwnedItemCounts()) do
        if G.TheItems:GetBarterSellPrice(k) > 0 then
            if (v > 1) then
				local quality = G.GetRarityForItem(k)
                if (common and quality == 'Common')
				or (classy and quality == 'Classy') 
				or (spiffy and quality == 'Spiffy') 
				or G.IsItemMarketable(k) ~= true then
                    if Queuer.getItemCount(k, v) > 1 then
